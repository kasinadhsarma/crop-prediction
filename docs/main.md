...
To combine the datasets `datasets/Crop_recommendation.csv` and `datasets/crop_yield.csv` into a merged dataset, follow these steps:

1. Ensure that both CSV files are present in the `datasets` directory.
2. Run the `backend/merge_datasets.py` script to merge the datasets.

### Example Command

```bash
python backend/merge_datasets.py
```

This will generate a new file `datasets/merged_dataset.csv` containing the merged data.
